 ...
 *
 * Enhanced version with full multi-protocol support:
 * - LF RFID (125kHz): HID H10301, EM4100, Indala, AWID
 * - NFC/HF (13.56MHz): NTAG, Mifare Classic (when NFC APIs available)
 * - Auto-detection and protocol switching
 * - Unified modulation field for cleaner UX
 *
...
/** Enhanced modulation/protocol selection */
typedef enum {
    Mod_Auto = 0,    /** Auto-detect all protocols (LF ASK+PSK, future NFC) */
    Mod_ASK,         /** LF ASK only (HID H10301, EM4100, most common) */
    Mod_PSK,         /** LF PSK only (Indala, AWID, less common) */
    Mod_NFC,         /** NFC 13.56MHz (NTAG, Mifare - when available) */
    Mod_COUNT
} ModSelect;
...
typedef struct {
    ...
    // TODO: Add NFC support when APIs are available
    // Nfc* nfc;
    ...
    ModSelect mod_select;            /** Modulation/protocol selection */
    ...
    /* Last read data */
    uint8_t last_data[32];           /** Raw protocol data from last read */
...
static void app_draw_config(Canvas* canvas, BulkWriterApp* app) {
    ...
    char fc_str[16], cn_str[32], mod_str[24];
    const char* cn_modes[] = {"Preserve", "Sequential", "Fixed"};
    const char* mod_names[] = {"Auto (All)", "ASK (125k)", "PSK (125k)", "NFC (13.56M)"};
    ...
    // Modulation/Protocol field (unified)
...
static void app_draw_ref_scan(Canvas* canvas, BulkWriterApp* app) {
    ...
    switch(app->mod_select) {
        ...
        case Mod_NFC: reader_msg = "NFC support coming soon!"; break;
        case Mod_Auto: reader_msg = "Place LF card on reader"; break;
    }
...
static void app_draw_ready(Canvas* canvas, BulkWriterApp* app) {
    canvas_clear(canvas);
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str_aligned(canvas, 64, 12, AlignCenter, AlignTop, "Bulk Writer V2");
    ...
    if(app->mod_select == Mod_NFC) {
        canvas_draw_str_aligned(canvas, 64, 40, AlignCenter, AlignCenter, "NFC support coming soon!");
    } else {
        ...
        case Screen_Config:
            switch(input_event->key) {
                    ...
                    switch(app->config_cursor) {
                        ...
                        case 2: // Modulation or Reference Scan
                            if(app->mod_select < Mod_COUNT - 1) {
                                ...
                                if(app->mod_select == Mod_NFC) {
                                    // NFC not supported yet - show message
                                    snprintf(app->error_msg, sizeof(app->error_msg), "NFC support coming soon!");
                ...
                case InputKeyOk:
                    ...
                    // Start LFRFID worker based on modulation setting
                    LFRFIDWorkerReadType read_type = LFRFIDWorkerReadTypeAuto;
...
static void save_settings(BulkWriterApp* app) {
    ...
    if(flipper_format_file_open_new(file, BULKWRITER_CONFIG_PATH)) {
        flipper_format_write_header_cstr(file, "BulkWriter V2 Config", 1);
...
static void process_read_card(BulkWriterApp* app) {
    ...
    if(!extract_success) {
        snprintf(app->error_msg, sizeof(app->error_msg), "Unsupported: %s", protocol_name);
        app->current_screen = Screen_Error;
    ...
    bool write_success = lfrfid_worker_write_start(app->lfrfid_worker, app->last_lfrfid_protocol);
            ...
            switch(event.type) {
                case EventTypeRead:
                    if(app->current_screen == Screen_RefScan) {
                        ...
                        app->ref_mod_type = app->mod_select;
 ...
 *  HID H10301 (26-bit): [1 parity][8 FC][16 CN][1 parity]
 *  EM4100:              [8-bit version/FC][32-bit ID] with row/col parity
 *  Future: NFC protocols when APIs are available
 *
 *  These helpers extract/inject facility code and card number from the
 *  protocol data structures.
 * ══════════════════════════════════════════════════════════════════════════════ */
...
static void app_draw_config(Canvas* canvas, BulkWriterApp* app) {
    ...
    char fc_str[16], cn_str[32], mod_str[24];
    const char* cn_modes[] = {"Preserve", "Sequential", "Fixed"};
    const char* mod_names[] = {"Auto (LF)", "ASK (125k)", "PSK (125k)", "NFC (Future)"};
    ...
    // Modulation field (unified)
    if(app->config_cursor == 2) canvas_draw_box(canvas, 0, y_pos - 9, 128, 11);
    canvas_draw_str(canvas, 2, y_pos, app->config_cursor == 2 ? ">" : " ");
    canvas_draw_str(canvas, 8, y_pos, "Mod: ");
    canvas_draw_str(canvas, 35, y_pos, mod_str);
    
    if(app->mod_select != Mod_NFC) {  // Only show scan option for implemented protocols
        canvas_draw_str(canvas, 90, y_pos, "Scan >");
    }
